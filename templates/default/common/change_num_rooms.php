        ...
        if(isset($_POST['num_rooms'][$room_id])){
            ...
            if($result_room !== false && $db->last_row_count() > 0){
                ...
                for($i = 0; $i < $num_rooms; $i++){
                    $response['html'] .= '
                    <div class="mb5 mt5 bg-success" style="margin-top:50px;">
                        ...
                        <div class="col-md-3 col-lg-2 pt5 pb5">
                            <div class="input-group input-group-sm">
                                ...
                                <select name="num_adults['.$room_id.']['.$i.']" class="nice-select chosen-select sendAjaxForm" data-extratarget="#booking-amount_'.$hotel_id.'" data-action="'.getFromTemplate('common/change_num_people.php').'?index='.$i.'&id_room='.$room_id.'&id_hotel='.$hotel_id.'" data-target="#room-result-'.$room_id.'-'.$i.'">
                        ...
                        <div class="col-md-3 col-lg-2 pt5 pb5" style="display:none;">
                            <div class="input-group input-group-sm">
                                <div class="input-group-addon"><i class="fas fa-fw fa-male"></i> '.$texts['CHILDREN'].'</div>
                                <select name="num_children['.$room_id.']['.$i.']" class="nice-select chosen-select sendAjaxForm" data-extratarget="#booking-amount_'.$hotel_id.'" data-action="'.getFromTemplate('common/change_num_people.php').'?index='.$i.'&id_room='.$room_id.'&id_hotel='.$hotel_id.'" data-target="#room-result-'.$room_id.'-'.$i.'">
                                    <option value="0">-</option>';
